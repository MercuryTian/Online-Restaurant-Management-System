...
.controller('MenuController', ['$scope', 'menuFactory', 'favoriteFactory', function ($scope, menuFactory, favoriteFactory) {
    ...
    menuFactory.query(
        ...
        function (response) {
            $scope.message = "Error1:" + response.status + " " + response.statusText;
        });
...
.filter('showfood', function () {
    return function (input, foodtype) {
        if(foodtype == 'appetizers'){
            var out = [];
            angular.forEach(input, function (food) {
                //console.log('food type', food);
                if (food.label == 'starter' || food.label == 'salad') {
                    out.push(food);
                    }
            });
            return out;
        }
        else if(foodtype == 'mains'){
            var out = [];
            angular.forEach(input, function (food) {
            if (food.label == 'chicken' || food.label == 'beef' || food.label == 'pork' || food.label == 'lamb' || food.label == 'seafood' || food.label == 'pizza') {
                out.push(food);
                }
            });
            return out;
            ...
            var out = [];
            angular.forEach(input, function (food) {
            if (food.label == 'dessert' || food.label == 'side dish') {
                out.push(food);
                }
            });
            return out;
...
.controller('DishDetailController', ['$scope', '$state', '$stateParams', 'menuFactory', 'commentFactory', function ($scope, $state, $stateParams, menuFactory, commentFactory) {
    ...
    $scope.dish = menuFactory.get({
        ...
        .$promise.then(
            ...
            function (response) {
                $scope.message = "Error12:" + response.status + " " + response.statusText;
            }
        );
...
.controller('FavoriteController', ['$scope', '$state', 'favoriteFactory', function ($scope, $state, favoriteFactory) {
    ...
    favoriteFactory.query(
        ...
        function (response) {
            $scope.message = "Error123456:" + response.status + " " + response.statusText;
        });
...
.controller('HeaderController', ['$scope', '$state', '$rootScope', 'ngDialog', 'AuthFactory', function ($scope, $state, $rootScope, ngDialog, AuthFactory) {
    ...
    $scope.openreserve = function() {
        ngDialog.open({ template: 'views/reserve.html', scope: $scope, className: 'ngdialog-theme-default', height: 400, width: 650});
    };
    //
